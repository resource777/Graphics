...
GLfloat star_body1[3][2] = { { -3.0, 0.0 },{ 3.0, 0.0 },{ 0.0, 5.0 } };
GLfloat star_body2[3][2] = { { -4.0, 2.5 },{ -1.0,2.5 } ,{ -3.0, 0.0 } };
GLfloat star_body3[3][2] = { { 4.0, 2.5 },{ 1.0, 2.5 } ,{ 3.0, 0.0 } };
GLfloat star_body4[3][2] = { { -3.0, 0.0 },{ 2.5, 0.0 },{ 0.0, -5.0 } };
GLfloat star_body5[3][2] = { { -4.0, -2.5 },{ -1.0, -2.5 } ,{ -3.0, 0.0 } };
GLfloat star_body6[3][2] = { { 4.0, -2.5 },{ 1.0, -2.5 } ,{ 3.0, 0.0 } };

GLfloat star_color[6][3] = {
	{ 255 / 255.0f, 255 / 255.0f, 0 / 255.0f },
{ 255 / 255.0f, 215 / 255.0f, 0 / 255.0f },
{ 255 / 255.0f, 255 / 255.0f, 224 / 255.0f },
{ 255 / 255.0f, 255 / 255.0f, 0 / 255.0f },
{ 255 / 255.0f, 215 / 255.0f, 0 / 255.0f },
{ 255 / 255.0f, 255 / 255.0f, 224 / 255.0f }
};
...
void prepare_star() {
	GLsizeiptr buffer_size = sizeof(star_body1) + sizeof(star_body2) + sizeof(star_body3) + sizeof(star_body4) + sizeof(star_body5) + sizeof(star_body6);
	...
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(star_body1), star_body1);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(star_body1), sizeof(star_body2), star_body2);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(star_body1) + sizeof(star_body2), sizeof(star_body3), star_body3);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(star_body1) + sizeof(star_body2) + sizeof(star_body3), sizeof(star_body4), star_body4);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(star_body1) + sizeof(star_body2) + sizeof(star_body3) + sizeof(star_body4), sizeof(star_body5), star_body5);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(star_body1) + sizeof(star_body2) + sizeof(star_body3) + sizeof(star_body4) + sizeof(star_body5), sizeof(star_body6),star_body6);
...
void draw_star() {
	...
	glUniform3fv(loc_primitive_color, 1, star_color[STAR_BODY1]);
	glDrawArrays(GL_TRIANGLE_FAN, 0, 3);

	glUniform3fv(loc_primitive_color, 1, star_color[STAR_BODY2]);
	glDrawArrays(GL_TRIANGLE_FAN, 3, 3);

	glUniform3fv(loc_primitive_color, 1, star_color[STAR_BODY3]);
	glDrawArrays(GL_TRIANGLE_FAN, 6, 3);

	glUniform3fv(loc_primitive_color, 1, star_color[STAR_BODY4]);
	glDrawArrays(GL_TRIANGLE_FAN, 9, 3);

	glUniform3fv(loc_primitive_color, 1, star_color[STAR_BODY5]);
	glDrawArrays(GL_TRIANGLE_FAN, 12, 3);

	glUniform3fv(loc_primitive_color, 1, star_color[STAR_BODY6]);
...
void display(void) {
	...
	//셔츠
	int shirt_angle = timestamp % 360;
	ModelMatrix = glm::rotate(glm::mat4(1.0f), shirt_angle * TO_RADIAN * 2.0f, glm::vec3(0.0f, 0.0f, 1.0f));
	ModelMatrix = glm::scale(ModelMatrix, glm::vec3(1.0f*shirt_angle/180, -1.0f*shirt_angle / 180, 0.0f));
	...
	{
		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(1000.0, 20.0f*((i+1)%8), 0.0f));
		ModelMatrix = glm::scale(ModelMatrix, glm::vec3(-1.0f, 1.0f, 0.0f));
		ModelMatrix = glm::translate(ModelMatrix, glm::vec3(car2_clock * 0.2f*((1+i)+(rand()%8)), 0.0f, 0.0f));
		//ModelMatrix = glm::scale(ModelMatrix, glm::vec3(car2_clock / 360.0, car2_clock / 360.0, 0.0f));
		ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
		glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
		draw_car2();
	}
	...
	for (int i = 1; i < 600; i++)
	{
		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-1000.0, -100.0 * 0.5*(i%2+i%3+i%5), 0.0f));
		ModelMatrix = glm::translate(ModelMatrix, glm::vec3(car2_clock * 0.6f * (1 + i), 0.6f, 0.0f));
	...
	//케이크 타원 회전
	int l_cake_angle = timestamp % 180;
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-420.0f+400.0f*cosf(l_cake_angle *TO_RADIAN), -250 + 200.0f*sinf(l_cake_angle *TO_RADIAN), 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, atanf( -1.0f / tanf(l_cake_angle * TO_RADIAN) / 4.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	ModelMatrix = glm::scale(ModelMatrix, glm::vec3(2.0f, 2.0f, 0.0f));
	ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
	glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
	draw_cake();
	int r_cake_angle = timestamp % 360;
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(420.0f + 400.0f * cosf(r_cake_angle * TO_RADIAN),-400.0f+200.0f * sinf(r_cake_angle * TO_RADIAN), 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, atanf(-1.0f / tanf(r_cake_angle * TO_RADIAN) / 4.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	ModelMatrix = glm::scale(ModelMatrix, glm::vec3(2.0*(1+(int)(r_cake_angle /180)), 2.0f*(1 + (int)(r_cake_angle / 180)), 3.0f));
	ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
	glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
	draw_cake();
	...
	for (int i = 0; i < 10; i++)
	{
		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(500.0f, 300.0f-10.0f*i, 0.0f));
	...
	//집
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(500.0f, 200.0f, 0.0f));
	...
	int windmill_angle= timestamp%360;
	...
	{
		Mtmp = glm::mat4(1.0f);	
		Mtmp = glm::translate(Mtmp, glm::vec3(500.0f, 300.0f, 0.0f));
		Mtmp = glm::rotate(Mtmp, (windmill_angle+90*i) * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
		Mtmp = glm::translate(Mtmp, glm::vec3(0, 0.0f, 0.0f));
		for (int j = 0; j < 5; j++)
		{
			ModelMatrix = glm::translate(Mtmp, glm::vec3(100, 0.0f, 0.0f));
			ModelMatrix = glm::rotate(ModelMatrix, 45.0f * j * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	//비행기
	int airplane_angle = timestamp % 360;
	Mtmp = glm::mat4(1.0f);
	Mtmp = glm::translate(Mtmp, glm::vec3(1000.0f, 100.0f, 0.0f));
	Mtmp = glm::rotate(Mtmp, 30 * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	for (int i = 0; i < 5; i++)
	{
		ModelMatrix = glm::translate(Mtmp, glm::vec3(-100*i+airplane_angle * -2.0f, airplane_angle * 2.0f * sqrt(3), 0.0));
		ModelMatrix = glm::scale(ModelMatrix, glm::vec3(airplane_angle / 60.0f, airplane_angle / 60.0f, 0.0f));
		ModelMatrix = glm::rotate(ModelMatrix, -150.0f * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
		ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
		glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
		draw_airplane();
	}
	...
	//칵테일
	int cock_angle = timestamp % 360;
	for (int i = 1; i <= 5; i++)
	{

		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-200*i+750.0f - 300.0f * (int)(cock_angle / 60.0f),(cock_angle % 60) * 5.0f+300, 0.0f));
		ModelMatrix = glm::rotate(ModelMatrix,30.0f*i * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
		ModelMatrix = glm::scale(ModelMatrix, glm::vec3(6 - 0.1f * (cock_angle % 60), 6 - 0.1f * (cock_angle % 60), 0.0f));
		ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
		glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
		draw_cocktail();
	}
	...
	for (int i = 0; i < 40; i++)
	{
		for (int j = 10; j < 30; j++)
		{
			ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-800.0f+40*i, 20*j, 0.0f));
			int exist = rand() % 100;
			ModelMatrix = glm::scale(ModelMatrix, glm::vec3(1.0f*!exist, 1.0f*!exist, 1.0f));
			ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
			glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
			draw_star();
		}
	...
	//별똥별
	int star_angle = timestamp % 40;
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-star_angle*10+100, 600 - star_angle*10, 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, -45 * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	ModelMatrix = glm::scale(ModelMatrix, glm::vec3(1, 6, 1.0f));
	ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
	glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
	draw_star();
	int star_angle2 = timestamp % 40;
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-star_angle2 * 15-50, 800 - star_angle2 * 15, 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, -45 * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	ModelMatrix = glm::scale(ModelMatrix, glm::vec3(1, 6, 1.0f));
	ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
	glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
	draw_star();
	int star_angle3 = timestamp % 20;
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-star_angle3 * 20-100, 600 - star_angle3 * 20, 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, -45 * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	ModelMatrix = glm::scale(ModelMatrix, glm::vec3(1, 6, 1.0f));
	ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
	glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
	...
	//선
	ModelMatrix = glm::scale(glm::mat4(1.0f), glm::vec3(10 , 1.0f , 1.0f));
	ModelMatrix = glm::translate(ModelMatrix, glm::vec3(0,190, 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, -45  * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
